...
<template>
  <div class="candidate-record">
    ...
    <div class="candidate-record-2">
      ...
      <div
        class="record-2"
        v-for="employees in getSearch.slice(0, options)"
        :key="employees.id"
      >
        <input class="candidate-1" type="checkbox" />
        <h2 class="candidate-2">
          <img :src="`${employees.imagePath}`" alt="" />{{ employees.name }}
        </h2>
        ...
        <h6 class="candidate-7 status" :style="statusUpdate(employees.status)">
          {{ employees.status }}
        </h6>
      </div>
      <div class="record-3">
...
label { color: #a2a1a8; margin-right: 20px }
@media screen and (max-width: 768px) {
  .record-1 h2,
  .record-2 h2,
  .status {
    font-size: var(--fs-aaaa);
  }
  .status {
    border-radius: 5px;
  }
  input[type="text"] {
    ...
    border-radius: 8px;
    font-size: var(--fs-aa);
  } .pi-search { font-size: var(--fs-aa);}
  label, option { font-size: var(--fs-vvv);}
}
</style>